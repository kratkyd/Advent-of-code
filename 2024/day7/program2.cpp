...
long long conc(long long a, long long b){
	return stoll(std::to_string(a).append(std::to_string(b)));
}

bool eval(long long& target, std::vector<long long>& vals, long long temp = 0, int index = 0){
	if(index == vals.size()-1){
		return (temp+vals[index] == target || temp*vals[index] == target || conc(temp, vals[index]) == target);
	} else {
		return (eval(target, vals, temp+vals[index], index+1) || eval(target, vals, temp*vals[index], index+1) || eval(target, vals, conc(temp, vals[index]), index+1));
	}
}
